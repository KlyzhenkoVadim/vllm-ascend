...
class ForwardTopMState:
    """
    Per-layer state for current req_ids batch.
    """
    num_topm_blocks: torch.Tensor | None = None#??? (num_reqs,)
    composite_bt: torch.Tensor | None = None # (num_reqs, N) <-- N = (num_topm_blocks_i) + chunk_size
    composite_indices: torch.Tensor | None = None # (num_reqs, N) --|
    topm_idxs: torch.Tensor | None = None # (num_reqs, topM) TODO: layout could be different (num_reqs, 1, topM) ?
