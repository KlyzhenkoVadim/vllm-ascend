...
class AscendDSAPrefillMetadata:
    ...
    act_qlen: int = 0               # actual query token count in current chunk
...
class AscendDSADecodeMetadata:
    ...
    topm_start_cache: torch.Tensor = None # [B] int: 0=default, 1=compute, 2=reuse
...
class AscendDSAMetadataBuilder(AttentionMetadataBuilder[AscendDSAMetadata]):
        ...
        **kwargs,
        ...
        if self.num_decodes > 0:
            decode_metadata = self.build_decode_metadata(
                ...
                input_batch=kwargs.get("input_batch", None),
                # index_topm=kwargs.get("index_topm", 550),
    ...
    def build_prefill_metadata(
        ...
        if self.prefill_ratio_to_sas_metadata.get("qli") is None:
            ...
            if has_topm:
                ...
                #TODO(KlyzhenkoVadim): Have a check what is there are several layers?!
            ...
            self.prefill_ratio_to_sas_metadata["qli"] = torch.ops._C_ascend.npu_vllm_quant_lightning_indexer_metadata(
                ...
                layout_query="TND",
                layout_key="PA_BSND",
                sparse_count=index_topm, #FIXME(KlyzhenkoVadim): HARDCODED
                sparse_mode=3,
                pre_tokens=(1 << 63) - 1,
                next_tokens=(1 << 63) - 1,
                cmp_ratio=4,
            ...
            self.prefill_ratio_to_sas_metadata["_has_cached"]=_has_cached
            self.prefill_ratio_to_sas_metadata["num_topm_blocks"]=num_topm_blocks
            self.prefill_ratio_to_sas_metadata["topm_unique_logical"]=topm_unique_logical
            self.prefill_ratio_to_sas_metadata["chunk_start_logical"]=chunk_start_logical
        ...
        _has_cached = self.prefill_ratio_to_sas_metadata["_has_cached"]
        num_topm_blocks = self.prefill_ratio_to_sas_metadata["num_topm_blocks"]
        topm_unique_logical = self.prefill_ratio_to_sas_metadata["topm_unique_logical"]
        chunk_start_logical = self.prefill_ratio_to_sas_metadata["chunk_start_logical"]

        return AscendDSAPrefillMetadata(
            attn_mask=None,
            query_lens=self.query_lens[reqs_start:].to(torch.int32),
            seq_lens=kvlens_key,
            context_lens=kvlens_key,
    ...
    def build_decode_metadata(
        ...
        #TODO(KlyzhenkoVadim): We need to remove it if we use TOPM!!!
        #TODO(KlyzhenkoVadim): We also need to do this logic (To create decode ratio to sas)
    ...
    def _build_topm_subgroups(self, input_batch, layer_name, seq_lens_np,
        ...
        req_ids = input_batch.req_ids[:B] #TODO: Тут нужно убедиться что req_ids идут сначал декоды!
...
class AscendDSAImpl(DSAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.indexer is not None:
            ...
            self.topk_idxs = torch.zeros(256, 1, self.index_topk, dtype=torch.int32, device=self.inderxer_wq_b.weight.device) #.fill_(-1)
        ...
        #TODO(KlyzhenkoVadim): Change to max_num_reqs
    ...
    def _forward_prefill(
        ...
        # We need to know which way we unpack indexer_k_cache!. But k_cache already in kv_cache!!!
        ...
        if self.compress_ratio > 1:
            ...
            #NOTE:(KlyzhenkoVadim): This is the output of Indexer
    ...
    def _forward_decode(
        ...
        if self.multistream_dsv4_dsa_overlap:
            ...
            # q
            # TODO(KlyzhenkoVadim): confirm this part calculation
            if _is_w8a8_dynamic(self.wq_b):
                if share_hs_quant:
                    q_a = torch_npu.npu_quant_matmul(
                        hs_int8,
    ...
    def _remap_composite_to_original(
        ...
        """Remap QLI output from composite [topM | chunk] to original address space.

        Composite layout indices:
          [0, topm_len) — top-M tokens, ordered as in topm_idxs.
          [topm_offset, topm_offset + act_qlen) — chunk sequential tokens.
          topm_offset = num_topm_blocks * block_size.
        """
        topm_flat = topm_idxs.squeeze(0).squeeze(0)  # (B,1,M) -> (M,)
    ...
    def _indexer_qli(
        ...
        if with_prefill:
            ...
            if default_meta is not None:
                ...
                #TODO(KlyzhenkoVadim): Have a check whether storing in tmp var is faster!
                # topk_idxs.index_copy_(0, compute_indices, c)
                topk_idxs.index_copy_(0,default_indices, default_topk_idxs)

            if compute_meta is not None:
                compute_indices = compute_meta.indices # TODO(KlyzhenkoVadim): Rename attribute
                compute_topm_idxs, _ = torch.ops._C_ascend.npu_vllm_quant_lightning_indexer(
                ...
                c =  compute_topm_idxs[:,:,:self.index_topk]
                topk_idxs.index_copy_(0, compute_indices, c)
                decode.topm_idxs.index_copy_(0, compute_indices, compute_topm_idxs)
    ...
    def _prepare_k_cache_for_qli(
        ...
        new_scale = torch_npu.npu_gather_sparse_index(s2d, flat_idx)
    ...
    def _prepare_k_cache_for_qli_prefill(
        ...
        act_qlen,            # actual token count in current chunk
    ):
