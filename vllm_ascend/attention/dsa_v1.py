...
class AscendDSAImpl(DSAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.indexer is not None:
            ...
            # IndexCache20: local KV cache for topM reuse
            # layer_name -> req_id -> topM index mapping (compressed_pos → global index)
    ...
    def _prepare_k_cache_for_qli(
        ...
        return (new_k, new_scale, topm_block_table)
    ...
    def _save_indexcache_anchor_topm_idxs(
        ...
        """Save per-request topM index mapping (compressed_pos -> global idx) to CPU cache."""
        if req_ids is None:
            return
    ...
    def _save_indexcache_anchor_topm_local_kv(
        ...
        """Gather topM K/scale from global cache and scatter into local fixed-size cache.

        Uses torch_npu.npu_gather_sparse_index for sparse gather and
        npu_scatter_nd_update_v2 for block-aligned scatter.
        """
        import math
        import torch_npu
    ...
    def _restore_indexcache_reuse_topk_idxs(
        ...
        """Map local topK indices (range [0, topM)) back to global compressed indices."""
        ...
        saved_topm_idxs = torch.stack(
            [layer_cache[req_id].to(local_topk_idxs_2d.device) for req_id in req_ids],
            dim=0,
        )
        ...
        return global_topk_idxs[:, None, :].to(local_topk_idxs.dtype) #FIXME: Do we really need this topm_block_table to compute here?!
