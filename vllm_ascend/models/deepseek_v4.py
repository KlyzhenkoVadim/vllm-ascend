...
class AscendDeepseekV4IndexerLocalCache(DeepseekV4IndexerCache):
    """Indexer local cache for IndexCache20 topM reuse.

    Uses FixedCacheSpec to allocate a fixed-size local KV cache
    per request that stores the topM K/scale values for reuse
    during non-anchor decode steps.
    """
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        return FixedCacheSpec(
            ...
            fixed_token_lengths=ascend_config.indexcache_buffer_len,
        )
...
class Indexer(nn.Module):
    def __init__(
        ...
        if self.compress_ratio == 4:
            ...
            from vllm_ascend.ascend_config import get_ascend_config
            ascend_config = get_ascend_config()
            if ascend_config.enable_local_k_cache:
